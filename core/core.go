...
const (
	...
	DIR_LEFT              = 4
	MAX_NUM_GAMES         = 100
	INACTIVE_GAME_TIMEOUT = time.Minute * 5
)

var MaxGamesReachedError = errors.New("maximum number of games reached")
...
func (e *Engine) CreateGame(userID string) (*Game, error) {
	e.Lock.Lock()
	defer e.Lock.Unlock()

	if e.numOfGames >= MAX_NUM_GAMES {
		return nil, MaxGamesReachedError
	}
...
func (e *Engine) GetGame(userID string) *Game {
	e.Lock.Lock()
	defer e.Lock.Unlock()

	game, _ := e.games[userID]

	return game
}

func (e *Engine) GetNumOfGames() int {
	e.Lock.Lock()
	defer e.Lock.Unlock()
...
func (e *Engine) RemoveGame(userID string) {
	e.Lock.Lock()
	defer e.Lock.Unlock()

	if _, exists := e.games[userID]; !exists {
		return
	}

	delete(e.games, userID)
	e.numOfGames--
}

func (e *Engine) CleanupInactiveGames() {
	e.Lock.Lock()
	defer e.Lock.Unlock()

	now := time.Now().UnixMilli()

	for userID, game := range e.games {
		game.Lock.Lock()

		if (now - game.LastInputTime) > INACTIVE_GAME_TIMEOUT.Milliseconds() {
			delete(e.games, userID)
			e.numOfGames--
		}
...
func NewGame(rows, cols int) *Game {
	return &Game{
		...
		LastInputTime:  time.Now().UnixMilli(),
		IsPaused:      false,
		ShowHelp:      true,
		Lock:          sync.Mutex{},
	}
...
func (state *Game) SetPlayerDir(dir int) {
	...
	state.LastInputTime = time.Now().UnixMilli()
}
